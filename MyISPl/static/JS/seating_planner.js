$(document).ready(function() {
    ...
    function setupContextMenu(chairDiv, chair) {
        chairDiv.off('contextmenu').on('contextmenu', function(e) {
            ...
            if (chair.studentId) {
                ...
                // Add Note option
                const addNoteOption = $('<div>Add Note</div>').click(function() {
                    showAddNoteDialog(chair.studentId);
                    contextMenu.remove();
                });
                contextMenu.append(addNoteOption);
            }
    ...
    function showAddNoteDialog(studentId) {
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="noteDetails" rows="3" required></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
        ...
        $('#saveNoteBtn').click(function() {
            const noteDetails = $('#noteDetails').val().trim();
            if (!noteDetails) {
            ...
            $.ajax({
                ...
                method: 'POST',
                contentType: 'application/json',
                data: JSON.stringify({ details: noteDetails }),
                success: function(response) {
                    modal.hide();
                    // Update student's note display if needed
                    updateStudentNote(studentId);
                },
                error: function(xhr, status, error) {
