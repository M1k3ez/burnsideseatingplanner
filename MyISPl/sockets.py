...
def check_sessions():
    while True:
        ...
        print(
            f"Background task is running: Checking sessions at {now.isoformat()}")
        ...
        for key in session_keys:
            ...
            session_expiry = datetime.fromisoformat(
                session_expiry_str.decode('utf-8'))
            print(
                f"User {user_id} session expiry time: {
                    session_expiry.isoformat()}")
            print(
                f"Comparing now >= session_expiry: {now >= session_expiry}")
            if now >= session_expiry and b'verification_due' not in session_data:
                ...
                print(f"Emitting 'request_verification' event to user {
                    user_id}")
                socketio.emit('request_verification', {
                    'message': 'Please verify your presence by entering your user ID.'}, room=user_id)
                ...
                print(
                    f"User {user_id} verification due time: {
                        verification_due.isoformat()}")
                if now >= verification_due:
                    # Verification failed, force logout
                    print(f"User {user_id} failed to verify. Logging out.")
                    socketio.emit(
                        'force_logout',
                        {'reason': 'Failed to verify presence.'}, room=user_id)
...
def handle_connect(auth):
    if not current_user.is_authenticated:
        ...
        print(
            f"Session expiry for user {user_id} set to {
                session_expiry_time.isoformat()}")
        print(f"Session data for user {user_id} saved in Redis: {
            session_data}")
...
def handle_verify_user(data):
    ...
    if provided_user_id == user_id:
        ...
        session_data = {
            ...
            'session_expiry': (
                datetime.now(timezone.utc) + SESSION_DURATION).isoformat(),
        }
        redis_client.hmset(session_key, session_data)
        redis_client.hdel(session_key, 'verification_due')
        emit('verification_success', {
            'message': 'Verification successful. Session extended.'})
        print(f"User {user_id} verified successfully. Session extended.")
    else:
        # Verification failed
        emit('verification_failed', {
            'message': 'Incorrect teacher code. You will be logged out in 5 seconds.'}, room=user_id)
        session_key = get_user_session_key(user_id)
        redis_client.delete(session_key)
        logout_user()
        disconnect()
